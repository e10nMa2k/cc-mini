"""WorkerManager — spawns and tracks background worker engines.

Each worker runs engine.submit() on a daemon thread, tracks progress,
and posts an XML <task-notification> to a Queue when done.
The REPL drains the queue between prompts and feeds notifications back
to the coordinator engine.
"""
